...
from HelperScripts.pubmed_cache_db import PubMedCacheDB
...
def run_backfill(
    ...
    def cache_getter(pmid: str) -> Optional[dict]:
        entry = cache_db.get_metadata(str(pmid).strip())
        if not entry:
            return None
        return {
    ...
    def cache_setter(
        ...
        *,
        ...
        if not pmid_clean.isdigit():
            return
        prior = cache_db.get_metadata(pmid_clean) or {}
        if apply:
            cache_db.upsert_metadata({
                "pmid": pmid_clean,
...
def main():
    ...
    parser.add_argument("--db-path", default="", help="SQLite PubMed cache DB path.")
    parser.add_argument(
        "--report-path",
    ...
    report = run_backfill(
        ...
        cache_db=PubMedCacheDB(args.db_path or None),
